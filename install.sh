...
sed -i "s/^[;[:space:]]*upload_max_filesize[[:space:]]*=.*/upload_max_filesize = 1024M/" "$INI"
sed -i "s/^[;[:space:]]*post_max_size[[:space:]]*=.*/post_max_size = 1050M/" "$INI"
sed -i "s/^[;[:space:]]*max_execution_time[[:space:]]*=.*/max_execution_time = 1800/" "$INI"
sed -i "s/^[;[:space:]]*max_input_time[[:space:]]*=.*/max_input_time = 1800/" "$INI"
sed -i "s/^[;[:space:]]*memory_limit[[:space:]]*=.*/memory_limit = 512M/" "$INI"

HASH="$(php -r "echo password_hash(getenv(\"ADMIN_PASS\"), PASSWORD_DEFAULT);" ADMIN_PASS="$ADMIN_PASS")"
sed -i "s|__ADMIN_HASH__|${HASH}|g" "$APP_DIR/admin.php"
...
## Install (Ubuntu 22, Apache port 8181)
```bash
